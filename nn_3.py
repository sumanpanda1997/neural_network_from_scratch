...
def driver(max_epochs, batch_size, learning_rate, num_layers, num_units, train_input,
          train_target, dev_input, dev_target):
...
def PCA(data):
    ...
    return np.dot(data, eigenvectors_sorted[:reduced_dimension].T)
...
def main():
    ...
    train_input=PCA(train_input)
    dev_input=PCA(dev_input)
    test_input=PCA(test_input)
